...
//                var jsontest = JSON.parse(r.data);
//                console.log(jsontest);
    ...
    //tokentx
    //https://blockscout.com/poa/core/api?module=account&action=tokentx&address=0x0Fe18f369c7F34208922cAEBbd5d21E131E44692
    tokenTxList(address, toAddress, callback){ //0x11c4469d974f8af5ba9ec99f3c42c07c848c861c  0x0Fe18f369c7F34208922cAEBbd5d21E131E44692  
        this.GETit("module=account&action=tokentx&address=" + address, function(r){
            if(r.success){
                ...
                for (var i in r.data.result) {
                    if (r.data.result[i].tokenSymbol == 'BUFF') {
                        //console.log(r.data.result[i].to);
                        //console.log(r.data.result[i].value);
                        if (r.data.result[i].to == toAddress) {
                            //console.log(r.data.result[i].value);
//                            console.log(r.data.result[i].timeStamp);
//                            timeStamps.push(r.data.result[i].timeStamp);
                            var dateTime = new Date(r.data.result[i].timeStamp*1000);
                            var dateTimeMST = new Date(r.data.result[i].timeStamp*1000).toLocaleString('en-US', {timeZone: 'America/Denver'})
//                            console.log(dateTimeMST);
                            timeStamps.push(dateTimeMST);
                            x += Number(r.data.result[i].value);
                        };
                    };
                };
...
//                console.log("preparing to parse");
//                return callback({success:true, data:JSON.parse(r.data.result)});
                return callback({success:true, data:y, timeStamps:timeStamps});
